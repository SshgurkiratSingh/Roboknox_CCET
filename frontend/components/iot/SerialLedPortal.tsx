...
export default function SerialLedPortal() {
  ...
  const headerBadge = useMemo(
    () =>
      isConnected ? (
        <span className="rounded-full bg-neon/15 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-neon">
          Live Session
        </span>
      ) : (
  ...
  if (!isSupported) {
    return (
      <div className="rounded-3xl border border-[#1a1a1a] bg-[#111111] p-8 text-white">
        ...
        <Link
          href="/"
          className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-neon"
        >
          ← Back to home
        </Link>
      </div>
  ...
  return (
    <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <div className="rounded-3xl border border-[#1a1a1a] bg-[#111111] p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-secondary">{statusMessage}</p>
          </div>
          {headerBadge}
        </div>
        ...
        <div className="mt-6 grid gap-4">
          ...
          <div className="rounded-2xl border border-[#1a1a1a] bg-[#0a0a0a] p-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <label className="text-xs text-secondary">Baud rate</label>
              <select
                value={baudRate}
                onChange={(event) =>
                  setBaudRate(Number(event.target.value) as typeof baudRate)
                }
                className="rounded-xl border border-[#1a1a1a] bg-[#0a0a0a] px-3 py-2 text-sm text-white"
              ...
              {!isConnected ? (
                <button
                  onClick={connect}
                  className="rounded-full border border-neon/40 px-4 py-2 text-sm font-semibold text-neon transition hover:border-emerald-300"
                >
                  Connect
                </button>
              ) : (
                    ...
                    {cube[activeLayer].map((row, y) =>
                      row.map((cell, x) => (
                        <button
                          ...
                          className={`w-full aspect-square rounded-lg border-2 transition hover:scale-105 ${cell
          ...
          {/* Send Section */}
          <div className="rounded-2xl border border-[#1a1a1a] bg-[#0a0a0a] p-4">
            <h3 className="text-sm font-semibold text-white mb-3">Send</h3>
            <button
              type="button"
              onClick={() => void sendCommand()}
              className="w-full rounded-full border border-neon/40 px-4 py-3 text-sm font-semibold text-neon transition hover:border-emerald-300 disabled:opacity-40"
              disabled={!isConnected}
            >
              📤 Send to LED Cube
            </button>
            <div className="mt-3 rounded-lg border border-[#1a1a1a] bg-[#0a0a0a] p-3">
              <p className="text-xs text-secondary">Hex payload:</p>
              <p className="text-xs text-neon font-mono break-all mt-1">
                {cubeToHex(cube)}
              </p>
            </div>
          </div>
      ...
      <div className="rounded-3xl border border-[#1a1a1a] bg-[#111111] p-6">
        ...
        <div className="mt-4 max-h-96 overflow-auto rounded-2xl border border-[#1a1a1a] bg-[#0a0a0a] p-4 text-xs text-slate-300">
          {logEntries.length === 0 ? (
            ...
            <ul className="space-y-2">
              {logEntries.map((entry) => (
                <li key={entry.id} className="flex gap-3">
                  <span className="text-secondary">{entry.timestamp}</span>
                  <span
                    className={
                      entry.type === "tx"
                        ? "text-neon"
        ...
        <div className="mt-4 flex gap-2">
          <button
            onClick={clearLog}
            className="flex-1 rounded-full border border-[#1a1a1a] px-3 py-2 text-xs font-semibold text-slate-300 transition hover:border-slate-500"
          >
            Clear log
          </button>
        </div>
        <div className="mt-4 rounded-2xl border border-[#1a1a1a] bg-[#0a0a0a] p-4">
          <h4 className="text-sm font-semibold text-white">Firmware Setup</h4>
          <p className="mt-2 text-xs text-secondary leading-relaxed">
            Your LED cube microcontroller needs firmware that:
          </p>
          <ul className="mt-2 space-y-1 text-xs text-secondary ml-2">
            <li>✓ Listens on serial (USB) at the baud rate selected above</li>
            <li>✓ Expects commands in format: <code className="text-neon">CUBE:hexstring</code></li>
    ...
    for (int y = 0; y < 3; y++) {
      for (int x = 0; x < 3; x++) {
        ...
        digitalWrite(ledPins[z][y][x], LOW);
              ...
              className="text-xs rounded px-2 py-1 border border-neon/40 text-neon hover:border-emerald-300 transition"
            >
              Copy Code
            </button>
          </div>
          ...
          <pre className="mt-2 bg-[#0a0a0a] rounded p-3 text-xs text-neon overflow-x-auto leading-relaxed font-mono max-h-80 text-[9px]">
    ...
    if (byteIdx * 2 < hex.length()) {
      ...
      digitalWrite(ledPins[z][y][x], state ? HIGH : LOW);
          ...
          <p className="mt-2 text-xs text-slate-300">
            ✓ Modify <code className="text-neon">ledPins</code> array to match your wiring
          </p>
          <p className="text-xs text-slate-300">
            ✓ Set <code className="text-neon">BAUD_RATE</code> to match your selection above
          </p>
          <p className="text-xs text-slate-300">
        ...
        <div className="mt-4 rounded-2xl border border-[#1a1a1a] bg-[#0a0a0a] p-4">
          <h4 className="text-sm font-semibold text-white">Upload Steps</h4>
          <ol className="mt-2 space-y-2 text-xs text-secondary list-decimal list-inside">
